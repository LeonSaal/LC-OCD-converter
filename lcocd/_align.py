...
def draw_plot(window:sg.Window, values:Mapping, num: str, path:str, corr: bool, offs:Mapping):
...
def align_window(path:str, corr:bool, offs:Mapping):
    ...
    slider = [[sg.T(signal), sg.Slider((-5,5), default_value=offs[signal],  resolution=.1,k=signal, enable_events=True, orientation='h', size=(15, 15))] for signal in signals]

    lo_ft = [[sg.T(f'{lang.same_folder}: {path}')],[sg.Table(values,headings=[lang.file, lang.time, *signals], key="-T_FILES-",
                expand_x=True,
                enable_click_events=True,
                enable_events=True,
                tooltip=lang.tip_ftable,
                num_rows=7,)]]
    frame_table =sg.Frame(lang.browse_files, layout=lo_ft, expand_x=True)

    lo_fs = [list(chain(*slider))+[sg.Push(),sg.B(lang.reset, k = '-RESET-'), sg.OK(k='-K-')],
            [sg.T(lang.curs), sg.Slider((0,120), default_value=60, resolution=0.1, k='x', orientation='h', enable_events=True)]]

    frame_slider = sg.Frame(lang.offs, layout=lo_fs, expand_x=True)
    ...
    while True:
        ...
        if values['-T_FILES-'] and values['-T_FILES-'][0] >=0:
            ...
            draw_plot(window=window, values=values, num=num,corr=corr, path=path, offs=new_offs)
